...
const makeCommunitiesSocket = (config) => {
    const felix = makeBusinessSocket(config)
    const { 
		authState, 
		ev, 
		query, 
    ...
    const communityFetchAllParticipating = async () => {
        ...
        felix.ev.emit('groups.update', Object.values(data))
        return data
    }
    ...
    felix.ws.on('CB:ib,,dirty', async (node) => {
        ...
        communityAcceptInviteV4: ev.createBufferedFunction(async (key, inviteMessage) => {
            ...
            await upsertMessage({
                key: {
                    ...
                    id: generateMessageID(felix.user?.id),
                    fromMe: false,
